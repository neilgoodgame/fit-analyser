...
def build_html_report(
    ...
    hdc,
    pdc,
    power_source: str = "instantaneous power",
    ...
    if has_hr_zones:
        ...
        for z in hr_zones:
            ...
            zone_data.append(
                {
                    "name": z["name"],
                    "description": z["description"].split("—")[0].strip()
                    if "—" in z["description"]
                    else z["description"],
                    "min": lo,
                    "max": hi,
    ...
    has_training_effect = any(
        v is not None for v in [aerobic_te, anaerobic_te, tss, if_val, np_val]
    )
    ...
    if has_lr and avg_left is not None:
        balance_note = (
            "balanced"
            if abs(avg_left - 50) < 1
            else ("left-dominant" if avg_left > 50 else "right-dominant")
        )
    ...
    core_min, core_avg, core_max = (
        temp_stats("core_temperature") if has_core else (None, None, None)
    )
    skin_min, skin_avg, skin_max = (
        temp_stats("skin_temperature") if has_skin else (None, None, None)
    )
    ...
    if has_elevation:
        ...
        elev_df["altitude_smooth"] = (
            elev_df["altitude"].rolling(window=30, center=True, min_periods=5).median()
        )
    ...
    ctx = {
        ...
        "normalized_power": int(np_val) if np_val is not None else None,
